...
class TestLutron(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_connection_injection(self) -> None:
        ...
        conn = LutronConnection(
            "host", "user", "pass", lambda x: None, connection_factory=mock_factory
        )
    ...
    @patch(
        ...
        read_data=b'<Project><GUID>G</GUID><OccupancyGroups/><Areas><Area Name="P" IntegrationID="0"><Areas/></Area></Areas></Project>',
    )
    def test_load_xml_db_from_cache(
        self, mock_file: MagicMock, mock_url: MagicMock
    ) -> None:
